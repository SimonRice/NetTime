...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getTimelineEntries(for complication: CLKComplication, before date: Date, limit: Int, withHandler handler: (@escaping ([CLKComplicationTimelineEntry]?) -> Void)) {
        ...
        let latestDate = date.addingTimeInterval(-1.0)
        let entries = self.getTimelineEntriesForComplication(complication,
            fromDate: self.earliestDate, toDate: latestDate, limit: limit)
        handler(entries)
    }
    ...
    func getTimelineEntries(for complication: CLKComplication, after date: Date, limit: Int, withHandler handler: (@escaping ([CLKComplicationTimelineEntry]?) -> Void)) {
        ...
        let earliestDate = date.addingTimeInterval(1.0)
        let entries = self.getTimelineEntriesForComplication(complication,
            fromDate: earliestDate, toDate: self.latestDate, limit: limit)
        handler(entries)
    }
